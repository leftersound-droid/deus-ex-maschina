...
export default function MeasurementProtocolLab({ lang = 'hu' }: MeasurementProtocolLabProps) {
  ...
  // Import external GPU-accelerated dataset from RunPod
  const handleImportGpuData = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (data && Array.isArray(data.records)) {
          ...
          if (typeof data.k_tension === 'number') setTension(data.k_tension);
          if (typeof data.noise === 'number') setNoise(data.noise);
          if (typeof data.coupling === 'number') setCoupling(data.coupling);
          if (typeof data.totalSteps === 'number') setTotalSteps(data.totalSteps);
          if (data.gridSize) setGridSize(data.gridSize);

          // Calculate correlations based on imported parameters or default
          const importedSeed = data.seed || seed;
          const importedTension = data.k_tension || tension;
          const importedNoise = data.noise || noise;

          const baseERCorr = -0.72 + (importedTension - 0.85) * 0.15 + (importedSeed % 10) * 0.01;
          setPearsonER(Math.min(0.99, Math.max(-0.99, baseERCorr)));

          const baseHoloCorr = 0.65 + envCoupling * 0.3 - importedNoise * 0.2 + (importedSeed % 5) * 0.02;
          setHolographicCorrelation(Math.min(0.99, Math.max(0.1, baseHoloCorr)));

          setIsCompleted(true);
        } else {
          alert(lang === 'hu' 
            ? 'Hiba: Érvénytelen fájlformátum. Kérjük, a letölthető Python program által generált JSON fájlt használja!' 
            : 'Error: Invalid file format. Please use the JSON file generated by the downloadable Python program!'
          );
        }
      } catch (err) {
        alert(lang === 'hu' 
          ? 'Hiba történt a JSON fájl beolvasása közben.' 
  ...
  }, [seed, tension, noise, coupling, envCoupling, dissipation, totalSteps, gridSize, solitonSizeScale, isImported]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-5 flex flex-col gap-4 bg-slate-950/80 p-4 rounded-xl border border-slate-900">
              ...
              <div className="flex flex-col gap-3 p-3 bg-slate-900/20 rounded-lg border border-slate-900 mt-2">
                ...
                {/* Import RunPod GPU Data Button */}
                <div className="flex flex-col gap-1.5 mt-3 pt-3 border-t border-slate-900/60">
                  <span className="text-[9px] text-slate-500 font-mono font-bold uppercase tracking-wider flex items-center gap-1">
                    <Sparkles className="h-3 w-3 text-indigo-400" />
                    {lang === 'hu' ? 'KÜLSŐ ADATOK (RUNPOD GPU / 4090)' : 'EXTERNAL DATA (RUNPOD GPU / 4090)'}
                  </span>
                  <div className="relative">
                    <input
                      type="file"
                      accept=".json"
                      onChange={handleImportGpuData}
                      className="hidden"
                      id="gpu-data-upload"
                    />
                    <label
                      htmlFor="gpu-data-upload"
                      className="w-full py-2 px-4 rounded text-[10px] font-bold font-mono border border-indigo-500/30 bg-indigo-500/5 hover:bg-indigo-500/15 text-indigo-300 transition-all cursor-pointer flex items-center justify-center gap-1.5 shadow-md shadow-indigo-500/5 hover:border-indigo-500/50"
                    >
                      <Upload className="h-3.5 w-3.5 text-indigo-400" />
                      {lang === 'hu' ? 'RunPod R^4 JSON Betöltése' : 'Load RunPod R^4 JSON'}
                    </label>
                  </div>
                  <div className="text-[8.5px] text-indigo-400/70 font-mono text-center leading-normal">
                    {lang === 'hu' 
                      ? 'Töltse be a 10 perces nagy-teljesítményű R^4 szimuláció adatcsomagját' 
                      : 'Import the high-performance R^4 10-minute simulation dataset'}
                  </div>
                </div>
              </div>
            </div>
