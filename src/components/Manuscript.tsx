...
export default function Manuscript({ lang = 'hu' }: ManuscriptProps) {
  ...
  const content = {
    hu: {
      ...
      sec1_p3_emergence: 'A gondolatkísérlet alapvető elképzelése az, hogy ha egy lehetőleg egyszerű, absztrakt téren értelmezünk egy rekurzív generálási szabályrendszert, akkor a létrejövő struktúra nem csupán magát a fizikai teret hozza létre, hanem azzal együtt új, kvázi független szabályrendszereket (törvényszerűségeket) is generál. Ebben a konkrét modellben például az ℝ⁴-ből az ℝ³-ra való áttéréskor egy természetes kényszerhatás miatt nemcsak egy újabb skalártér (a feszültségi mező) keletkezik, hanem mellette egy az eredeti diffúziós szabályoktól nemlineárisan függő, új típusú kölcsönhatás is kialakul. A hipotézis lényege, hogy ezek a másodlagos, emergáló kölcsönhatások valóban új minőséget képviselnek, és nem egyszerűen a háttérben futó eredeti szabályszerűségek másolatai az altereken.',
...
# Ez létrehoz egy 'dist' nevű mappát, ami tartalmazza a teljes weboldalt.`,
    ...
    en: {
      ...
      sec1_p3_emergence: 'The fundamental idea of this thought experiment is that when we define a recursive generation rule on a maximally simple, abstract space, the resulting structure does not merely create the physical space itself, but also brings forth entirely new, quasi-independent systems of laws. In this specific model, for instance, during the transition from ℝ⁴ to ℝ³ under a natural constraint, not only does a new scalar field (the tension field) emerge, but alongside it, a new type of interaction arises that depends non-linearly on the original rules. The core hypothesis is that these secondary, emergent interactions represent genuinely novel phenomena, rather than being simple copies of the original rules acting on lower-dimensional subspaces.',
...
# This creates a 'dist' folder containing the entire website.`,
    ...
    de: {
      ...
      sec1_p3_emergence: 'Die grundlegende Idee dieses Gedankenexperiments besteht darin, dass wenn wir eine rekursive Erzeugungsregel auf einem möglichst einfachen, abstrakten Raum definieren, die resultierende Struktur nicht nur den physikalischen Raum selbst erschafft, sondern auch völlig neue, quasi-unabhängige Gesetzmäßigkeiten hervorbringt. In diesem spezifischen Modell entsteht beispielsweise beim Übergang von ℝ⁴ zu ℝ³ unter einem natürlichen Zwang nicht nur ein neues Skalarfeld (das Spannungsfeld), sondern daneben auch eine neue Art von Wechselwirkung, die nichtlinear von den ursprünglichen Regeln abhängt. Die Kernhypothese ist, dass diese sekundären, emergenten Wechselwirkungen echte neue Qualitäten darstellen und nicht bloß einfache Kopien der ursprünglichen Regeln auf Unterräumen sind.',
          ...
          {expandedSection === 'phil' && (
            <div id="sec-philosophy-content" className="p-6 border-t border-slate-850 bg-slate-950/15 text-sm leading-relaxed flex flex-col gap-4 text-slate-350">
              ...
              <div>
                ...
                <p className="mt-3 border-l-2 border-sky-500/40 pl-3 text-slate-300 bg-sky-500/5 p-2 rounded-r-lg">{t.sec1_p3_emergence}</p>
              </div>
